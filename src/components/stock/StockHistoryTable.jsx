import { FiArrowDown, FiArrowUp, FiClock } from 'react-icons/fi'
import { Link } from 'react-router-dom'

import { formatOrderDate } from '../../utils/orderFormatters.js'

function StockHistoryTable({ movements, total, loading }) {
  return (
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm shadow-slate-200/40">
      <div className="border-b border-slate-100 px-5 py-4">
        <h2 className="font-bold text-slate-900">Stock Movement History</h2>
        <p className="mt-0.5 text-xs text-slate-500">
          {total} {total === 1 ? 'movement' : 'movements'} found
        </p>
      </div>

      {loading ? (
        <div className="grid place-items-center px-6 py-16"><span className="size-9 animate-spin rounded-full border-4 border-primary-light border-t-primary" /><p className="mt-3 text-sm text-slate-500">Loading history...</p></div>
      ) : movements.length ? (
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1320px] text-left">
            <thead>
              <tr className="bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500">
                <th className="px-5 py-3">Date</th>
                <th className="px-4 py-3">Item</th>
                <th className="px-4 py-3">Type</th>
                <th className="px-4 py-3 text-right">Quantity</th>
                <th className="px-4 py-3 text-right">Previous Stock</th>
                <th className="px-4 py-3 text-right">New Stock</th>
                <th className="px-4 py-3">Reference</th>
                <th className="px-4 py-3">Supplier</th>
                <th className="px-4 py-3">Reason</th>
                <th className="px-5 py-3">User</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {movements.map((movement) => (
                <tr key={movement.id} className="text-sm text-slate-600 hover:bg-slate-50/70">
                  <td className="whitespace-nowrap px-5 py-4">
                    {formatOrderDate(movement.date, true)}
                  </td>
                  <td className="whitespace-nowrap px-4 py-4 font-semibold text-slate-800">
                    {movement.itemId ? <Link to={`/stock/history?item=${movement.itemId}`} className="record-link" title="Filter history by this item">{movement.itemName}</Link> : movement.itemName}
                  </td>
                  <td className="whitespace-nowrap px-4 py-4">
                    <span
                      className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${
                        movement.type === 'Stock In'
                          ? 'bg-emerald-50 text-emerald-700'
                          : 'bg-rose-50 text-rose-700'
                      }`}
                    >
                      {movement.type === 'Stock In' ? <FiArrowDown /> : <FiArrowUp />}
                      {movement.type}
                    </span>
                  </td>
                  <td className="px-4 py-4 text-right font-bold text-slate-800">
                    {movement.quantity} {movement.unit}
                  </td>
                  <td className="px-4 py-4 text-right">{movement.previousStock} {movement.unit}</td>
                  <td className="px-4 py-4 text-right font-semibold text-slate-800">
                    {movement.newStock} {movement.unit}
                  </td>
                  <td className="whitespace-nowrap px-4 py-4">
                    {movement.orderId ? <Link to={`/orders/${movement.orderId}`} className="record-link" title="View order">{movement.reference}</Link> : movement.purchaseId ? <Link to={`/purchases/${movement.purchaseId}`} className="record-link" title="View purchase">{movement.reference}</Link> : movement.reference}
                  </td>
                  <td className="whitespace-nowrap px-4 py-4">
                    {movement.supplierId && movement.supplierName ? <Link to={`/suppliers/${movement.supplierId}`} className="record-link" title="View supplier">{movement.supplierName}</Link> : '—'}
                  </td>
                  <td className="whitespace-nowrap px-4 py-4">{movement.reason}</td>
                  <td className="whitespace-nowrap px-5 py-4">{movement.userName || 'Not available'}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      ) : (
        <div className="grid place-items-center px-6 py-16 text-center">
          <span className="grid size-12 place-items-center rounded-full bg-slate-100 text-xl text-slate-400">
            <FiClock />
          </span>
          <p className="mt-3 font-semibold text-slate-700">No stock history found</p>
          <p className="mt-1 text-sm text-slate-500">Stock movements will appear here.</p>
        </div>
      )}
    </section>
  )
}

export default StockHistoryTable
