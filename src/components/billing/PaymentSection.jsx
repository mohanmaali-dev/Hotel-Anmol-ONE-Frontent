...
function PaymentSection({ bill, onUpdate, updating, canEdit }) {
  ...
  const savePayment = async (amount = paidAmount) => {
    await onUpdate({ paymentType, paidAmount: Number(amount) })
  }
    ...
    <section className="print-card rounded-xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/40">
      ...
      {canEdit ? (
        <div className="mt-5 space-y-5 print:hidden">
          ...
          <label><span className="mb-1.5 block text-sm font-semibold text-slate-700">Paid Amount</span><span className="relative block"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">₹</span><input type="number" min="0" max={bill.finalAmount} step="0.01" value={paidAmount} disabled={updating} onChange={(event) => setPaidAmount(event.target.value)} className="h-10 w-full rounded-lg border border-slate-200 bg-white pl-8 pr-3 text-sm text-slate-700 outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/10 disabled:bg-slate-50" /></span></label>
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
            ...
            <button type="button" onClick={() => savePayment()} disabled={updating || paidAmount === ''} className="flex h-10 items-center justify-center gap-2 rounded-lg bg-primary px-3 text-sm font-semibold text-white hover:bg-primary-dark disabled:cursor-not-allowed disabled:opacity-60"><FiSave /> {updating ? 'Updating...' : 'Update Payment'}</button>
          </div>
        </div>
