    ...
    <article className="print-bill hidden text-slate-900 print:block">
      <header className="flex items-start justify-between gap-8 border-b-2 border-primary pb-4">
        ...
        <div className="shrink-0 text-right">
          <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-primary-dark">Restaurant Bill</p>
          <p className="mt-1 text-lg font-bold">#{bill.billNo}</p>
          <p className="mt-1 text-[10px] text-slate-500">{formatOrderDate(bill.date, true)}</p>
        </div>
      </header>
      ...
      <section className="grid grid-cols-3 gap-x-8 gap-y-3 border-b border-slate-200 py-4">
        {[
          ...
          <div key={label}>
            ...
            <p className={`mt-1 text-[10px] font-bold ${label === 'Payment Status' ? statusClasses[bill.paymentStatus] : ''}`}>{value}</p>
          </div>
        ))}
      </section>
      ...
      <section className="print-summary mt-5 grid grid-cols-2 gap-10">
        <div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-x-5 gap-y-3 text-[10px]">
            <div><p className="text-[8px] uppercase text-slate-400">Payment Type</p><p className="mt-1 font-bold">{bill.paymentType || '—'}</p></div>
            <div><p className="text-[8px] uppercase text-slate-400">Status</p><p className={`mt-1 font-bold ${statusClasses[bill.paymentStatus]}`}>{bill.paymentStatus}</p></div>
            <div><p className="text-[8px] uppercase text-slate-400">Paid Amount</p><p className="mt-1 font-bold text-emerald-700">{formatCurrency(bill.paidAmount, currency)}</p></div>
            <div><p className="text-[8px] uppercase text-slate-400">Due Amount</p><p className="mt-1 font-bold text-rose-700">{formatCurrency(bill.dueAmount, currency)}</p></div>
          </div>
        </div>

        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-[10px]">
          <div className="flex justify-between gap-5 py-1"><span className="text-slate-500">Subtotal</span><strong>{formatCurrency(bill.subtotal, currency)}</strong></div>
          <div className="flex justify-between gap-5 py-1"><span className="text-slate-500">Discount</span><strong className="text-emerald-700">- {formatCurrency(bill.discount, currency)}</strong></div>
          <div className="flex justify-between gap-5 py-1"><span className="text-slate-500">Additional Charges</span><strong>{formatCurrency(bill.additionalCharges, currency)}</strong></div>
          <div className="mt-2 flex items-center justify-between gap-5 border-t border-slate-300 pt-2"><span className="font-bold">Final Amount</span><strong className="text-sm text-primary-dark">{formatCurrency(bill.finalAmount, currency)}</strong></div>
        </div>
      </section>
