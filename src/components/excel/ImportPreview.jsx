...
function ImportPreview({ preview, confirmed, onConfirmChange }) {
  ...
  return <div className="mt-4 overflow-hidden rounded-lg border border-slate-200 bg-slate-50/50">
    <div className="flex items-start gap-2 border-b border-slate-200 bg-white px-3.5 py-3"><FiEye className="mt-0.5 shrink-0 text-primary" /><div><p className="text-sm font-semibold text-slate-800">File Preview</p><p className="mt-0.5 text-xs leading-5 text-slate-500">{preview.note} {preview.validRows.length} valid row{preview.validRows.length === 1 ? '' : 's'}.</p></div></div>
    ...
    {preview.errors.length > 0 && <div className="max-h-36 overflow-y-auto border-t border-rose-100 bg-rose-50 px-3.5 py-3 text-xs text-rose-700"><p className="mb-1.5 font-semibold">Rows that need attention ({preview.errors.length})</p>{preview.errors.map((error, index) => <p key={`${error.row}-${index}`}>Row {error.row}: {error.message}</p>)}</div>}
