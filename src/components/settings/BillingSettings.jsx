  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/40 sm:p-6">
      ...
      <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <label><span className="mb-1.5 block text-sm font-semibold text-slate-700">Tax Percentage</span><div className="relative"><input type="number" min="0" max="100" step="0.01" value={settings.taxPercentage} onChange={(event) => onChange('taxPercentage', event.target.value)} className={`${inputClass} pr-9`} /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-slate-400">%</span></div></label>
