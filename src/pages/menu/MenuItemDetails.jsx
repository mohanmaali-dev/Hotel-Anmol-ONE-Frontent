...
function MenuItemDetails() {
  ...
  return (
    <main className="px-4 py-6 sm:px-6 lg:px-8 lg:py-8"><div className="page-content">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/40 sm:p-6"><div className="flex items-center gap-2"><span className="grid size-8 place-items-center rounded-lg bg-primary-light text-primary-dark"><FiTag /></span><h3 className="font-bold text-slate-900">Item Information</h3></div><div className="mt-5 grid grid-cols-2 gap-x-5 gap-y-6 sm:grid-cols-3 lg:grid-cols-5"><DetailItem label="Item Name" value={item.name} /><DetailItem label="Category" value={category?.name} /><DetailItem label="Selling Price" value={formatCurrency(item.sellingPrice)} /><DetailItem label="Availability" value={available ? 'Available' : 'Unavailable'} /><DetailItem label="Track Stock" value={item.trackStock ? 'Yes' : 'No'} /><div className="col-span-2 sm:col-span-3 lg:col-span-5"><DetailItem label="Description" value={item.description} /></div></div>{category && !category.isActive && <p className="mt-4 rounded-lg bg-amber-50 px-3 py-2.5 text-xs text-amber-700">This item is unavailable because its category is disabled.</p>}</section>
      ...
      <ConfirmDeleteModal open={confirmDelete} title={`Delete ${item.name}?`} message="This menu item and its recipe will be permanently removed. Deletion may be blocked if existing orders use this item." confirmLabel="Delete Menu Item" loading={working} onConfirm={handleDelete} onClose={() => setConfirmDelete(false)} />
    </div></main>
  )
}
