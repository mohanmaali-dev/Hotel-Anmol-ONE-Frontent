...
function Stock() {
  ...
  return (
    <main className="px-4 py-6 sm:px-6 lg:px-8 lg:py-8"><div className="page-content">
      ...
      <ConfirmDeleteModal open={Boolean(pendingDelete)} title={`Delete ${pendingDelete?.name || 'stock item'}?`} message="This stock item will be permanently removed. Its current quantity must be zero and it cannot be used by recipes or active purchases." confirmLabel="Delete Stock Item" loading={deleting} onConfirm={handleDelete} onClose={() => setPendingDelete(null)} />
    </div></main>
  )
}
